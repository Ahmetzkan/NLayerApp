...
namespace WebAPI.Controllers
{
    ...
    public class CustomersController : ControllerBase
    {
        ...
        {
            var result = await _customerService.AddAsync(createCustomerRequest);
            return Ok(result);
        } 
        [HttpPost("Update")]
        public async Task<IActionResult> Update([FromBody] UpdateCustomerRequest updateCustomerRequest)
        {
            var result = await _customerService.UpdateAsync(updateCustomerRequest);
            return Ok(result);
        } 
        [HttpPost("Delete")]
        public async Task<IActionResult> Delete([FromBody] DeleteCustomerRequest deleteCustomerRequest)
        {
            var result = await _customerService.DeleteAsync(deleteCustomerRequest);
            return Ok(result);
        }
        [HttpGet("GetList")]
